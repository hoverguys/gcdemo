...
void TextureResource::Initialize() {
    header = (TextureResourceHeader*)data.data();
    unsigned char* rawdata = (unsigned char*)&data[header->dataOffset];
    ...
    std::printf("Loading texture (%dx%d fmt %d wrap S%x T%x filter %x) at offset %p\n", header->width, header->height,
                header->format, header->wrapS, header->wrapT, header->filter, rawdata);

    // Check for palette format
    if (header->format >= 0x08 && header->format <= 0x0A) {
        palette = (unsigned char*)&data[header->paletteOffset];
    }
