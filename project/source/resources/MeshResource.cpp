...
void MeshResource::Initialize() {
    header = (MeshResourceHeader*)data.data();
    unsigned char* base = (unsigned char*)data.data();
    ...
    auto m = eastl::make_shared<Mesh>(Mesh(
        reinterpret_cast<Vector*>(base + posOffset), reinterpret_cast<Vector*>(base + nrmOffset),
        reinterpret_cast<float*>(base + texOffset), reinterpret_cast<MeshIndex*>(base + indOffset), header->fcount));
